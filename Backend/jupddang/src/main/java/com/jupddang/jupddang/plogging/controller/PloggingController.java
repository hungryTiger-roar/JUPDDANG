...
public class PloggingController {
    ...
    @Operation(summary = "플로깅 결과 임시 저장", description = "플로깅 결과 화면에서 작성 중인 내용을 임시 저장합니다. " + "data 파라미터는 PloggingEndRequest JSON 문자열이어야 합니다.")
    public ResponseEntity<PloggingTempSaveResponse> savePloggingTemp(
        ...
        PloggingEndRequest request =
                objectMapper.readValue(dataJson, PloggingEndRequest.class);

        // 서비스 호출
        PloggingTempSaveResponse response =
                ploggingService.savePloggingTemp(
                        account.getUserId(),
                        request,
                        beforeImage,
                        afterImage,
