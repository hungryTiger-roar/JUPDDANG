...
class Party {
  ...
  factory Party.fromJson(Map<String, dynamic> json) {
    ...
    // members가 Map일수도, List일수도 있음 (테스트 코드/API 명세 차이 대응)
    ...
    if (membersData is Map<String, dynamic>) {
      final list = membersData['list'] as List?;
      membersList = list?.map((m) => PartyMember.fromJson(m as Map<String, dynamic>)).toList() ?? [];
      current = membersData['current'] ?? 0;
      max = membersData['max'] ?? 4;
    } else if (membersData is List) {
      membersList = membersData.map((m) => PartyMember.fromJson(m as Map<String, dynamic>)).toList();
      current = membersList.length;
    }
