...
class AuthService {
  ...
  static const String followBase = '$apiBase/follow';

  /// 팔로워/팔로잉 숫자 조회
  Future<Map<String, int>> getFollowStats(String userId) async {
    try {
      final followersFuture = getFollowers(userId);
      final followingFuture = getFollowings(userId);
      ...
      print('Get Follow Stats Error: $e');
  ...
  /// 팔로우 여부 확인 (내 팔로잉 목록에 대상이 있는지 확인)
  Future<bool> checkIsFollowing(String targetUserId) async {
    if (accessToken == null || userId == null) return false;
    try {
      // 내 팔로잉 목록 조회
      final myFollowings = await getFollowings(userId!);
      // 목록 중에 targetUserId가 있는지 확인
      // FollowResponse 구조: {userId: "...", ...}
      return myFollowings.any((user) => user['userId'] == targetUserId);
    } catch (e) {
      return false;
    }
  }
